<template>
	<div class="p-4">
		...
		<div class="mt-2">
			...
			<input v-model="newTag" @keyup.enter="addTag" placeholder="Add a tag" class="mt-2 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm" />
		</div>
		...
		<div class="mt-4">
			...
			<div v-else>
				<div v-for="tag in selectedTags" :key="tag" class="inline-block bg-blue-500 text-white rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2">
					{{ tag }}
