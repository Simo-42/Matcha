		...
		<div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
			...
			<form @submit.prevent="submitBirthdate" class="space-y-6">
				<div>
					...
					<input type="date" v-model="birthdate" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required />
				</div>
				<div>
...
const submitBirthdate = async () => {
	...
	try {
		const response = await axios.post(
			"http://localhost:3005/api/after_auth/profil/update_birthdate",
			{
				age,
			},
			{
