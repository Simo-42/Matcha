		...
		<div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
			...
			<form @submit.prevent="modify_profil" class="space-y-6">
				...
				<div>
					...
					<small v-if="new_password && !isPasswordComplex" class="text-red-500"> Password must be at least 8 characters long, include an uppercase letter, a number, and a special character. </small>
					<small v-if="new_password && isPasswordComplex" class="text-green-500"> Password is strong enough. </small>
				</div>
				<div>
...
const modify_profil = async () => {
	try {
		const response = await axios.post(
			"http://localhost:3005/api/after_auth/profil/change_password",
			{
