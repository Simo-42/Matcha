<template>
	<div class="p-4">
		<div>
			...
			<select id="orientation" v-model="localOrientation" @change="emitOrientation" class="mt-2 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm bg-gray-200 text-gray-700">
...
const emitOrientation = () => {
	// Pour mettre à jour l'orientation
	emit("updateSexualPref", localOrientation.value);
};

watch(
	() => props.selectedSexualPref,
	(newVal) => {
