    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
      ...
      <form @submit.prevent="resetPassword" class="space-y-6">
        <div>
          ...
          <input type="password" v-model="newPassword" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
        </div>
        <div>
...
const resetPassword = async () => {
  ...
  try {
    const response = await axios.post(`http://localhost:3000/api/password/reset/${resetToken}`, {
