...
router.post("/profil/update", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "User spec updated successfully", user });
  } catch (error) {
...
router.post("/profil/update_profil", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (await userQueries.check_username_user_exist(username)) {
      const existingUsernameUser = await userQueries.getUserByUsername(
        username
      );
    ...
    if (email !== email_check.email) {
      ...
      return res.status(200).json({
        message:
          "Don't forget to verify your mail again. Mail has been sent to you",
        user,
      });
    }
...
router.get("/profil/spec_info", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "User information fetched successfully", result });
  } catch (error) {
...
router.post("/profil/change_password", authenticateToken, async (req, res) => {
  ...
  try {
    if (
      (await userQueries.check_same_password(username, new_password)) == true
    ) {
      // Si le nouveau mot de passe est le même que l'ancien
      return res.status(400).json({ error: "Same password as before." });
    } else if (
      (await userQueries.check_same_password(username, password)) == true
    ) {
      // Si le mot de passe actuel est correct
      const hashedPassword = await bcrypt.hash(new_password, 10);
      const user = await userQueries.update_user_password(
        username,
        hashedPassword
      );
      if (!user) {
      ...
      return res
        .status(200)
        .json({ message: "Your password has been modified", user });
    }
  } catch (error) {
    console.log("Error updating user:", error);
    return res.status(500).json({ error: "Internal server error" });
...
router.post("/profil/update_birthdate", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Your age has been updated successfully.", user });
  } catch (error) {
    console.log("Error updating user:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
...
router.get("/profil/profil_complete", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "User already complete his profil", result });
  } catch (error) {
    console.log("Error fetching user:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
...
router.get("/profil/picture_info", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "User  pics retrieved successfully profil", result });
  } catch (error) {
    console.log("Error fetching user:", error);
    return res.status(500).json({ error: "Internal server error" });
  }
