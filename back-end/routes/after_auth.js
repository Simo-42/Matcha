...
router.post('/profil/change_password', authenticateToken, async (req, res) => {
	...
	try 
	{
		if (await userQueries.check_same_password(username, new_password) == true) // Si le nouveau mot de passe est le même que l'ancien
		{
			return res.status(400).json({ error: 'Same password as before.' });
		}
		else if (await userQueries.check_same_password(username, password) == true) // Si le mot de passe actuel est correct
		{
...
router.get('/profil/profil_complete', authenticateToken, async (req, res) => { // token profile complete
	const userId = req.user.id;  // Utilisation de `req.user` pour accéder à l'ID utilisateur
	console.log('User ID:', userId);
...
router.get('/profil/my_profil_info', authenticateToken, async (req, res) => { // token profile complete
	const userId = req.user.id;  // Utilisation de `req.user` pour accéder à l'ID utilisateur
	console.log('User ID:', userId);
