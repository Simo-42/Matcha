...
router.post("/pictures", authenticateToken, upload.array("pictures", 5), async (req, res) => {
	...
	try {
		...
		console.log("User profile completion status updated successfully:", update_user);
		return res.status(200).json({ message: "User photos updated successfully", user });
	} catch (error) {
...
router.post("/update_pics", authenticateToken, upload.array("pictures", 5), async (req, res) => {
	const userId = req.user.id; // Utiliser `req.user` pour accéder aux informations de l'utilisateur
	console.log("Received files:", req.files);
	console.log("User ID:", req.user.id);
...
router.get("/get_pics", authenticateToken, async (req, res) => {
	...
	try {
		...
		return res.status(200).json({ message: "Photos fetched successfully", photos });
	} catch (error) {
