...
router.get('/profil_to_match', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!getProfilesFunction) {
            ...
            return res.status(400).json({ error: `No profiles found for gender ${user.gender} and preference ${user.sexual_preference}` });
        }
...
const filterAndSortProfiles = (profiles, searchCriteria) => {
    ...
    if (age_gap) {
        ...
        profiles = profiles.filter(profile => profile.age >= age_min && profile.age <= age_max);
    }
