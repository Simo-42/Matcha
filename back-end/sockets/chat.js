...
module.exports = (io, socket) => {
	...
	socket.on("userTyping", (data) => {
		try {
		  ...
		  // Vérifie que roomId et userId sont bien présents
		  if (!userId || !roomId) {
		  ...
		  console.log(`L'utilisateur ${userId} est en train d'écrire dans la room ${roomId}`);
		} catch (error) {
		  console.error("Error during user typing event:", error);
		  socket.emit("Error message", { message: "Une erreur s'est produite lors de la gestion de l'événement userTyping." });
		}
	  });
	...
	socket.on("joinRoom", (data) => {
		try {
			...
			socket.emit("Error message", { message: "Une erreur s'est produite lors de la connexion à la room." });
		}
	});
};
