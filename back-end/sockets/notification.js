	...
	socket.on("Notification", async (data) => {
	  try {
		if (data.status === "addNotification") {
		  ...
		  const result = await userQueries.AddNotification(data.userId, data.type, data.message);
		...
		socket.emit("ErrorNotification", { message: "Une erreur s'est produite lors du traitement de la notification." }); // Émettre une erreur à l'utilisateur
	  }
	});
  };
