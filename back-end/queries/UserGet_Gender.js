...
const getWomenAndBisexualAndGayMenProfiles = async (userId) => {
    try {
        ...
        const res = await pool.query(query, values);
        return res.rows;  // Retourne les femmes hétérosexuelles/bisexuelles, puis les hommes bisexuels et homosexuels (gay)
    } catch (err) {
        console.error('Error executing query', err.stack);
        throw err;
    }
...
const getMenAndBisexualAndGayWomenProfiles = async (userId) => {
    try {
        ...
        const res = await pool.query(query, values);
        return res.rows;  // Retourne les hommes hétérosexuels/bisexuels, puis les femmes bisexuelles et homosexuelles (gay)
    } catch (err) {
        console.error('Error executing query', err.stack);
        throw err;
    }
...
module.exports = {getHeterosexualWomenProfiles, 
    getHeterosexualMenProfiles, getMenAndBisexualAndGayWomenProfiles, 
    getOtherProfiles, getWomenAndBisexualAndGayMenProfiles, getGayMenProfiles,
    getGayWomenProfiles };
